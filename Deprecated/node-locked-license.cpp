...
#if _WIN32
    status = SetProductId(L"96d27e32-4e08-4a80-a332-2b4446613c02", LA_USER);
#else
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 4)
    {
        ...
        if (string(argv[1]) == "--activate")
        {
            ...
            {
                printf("Activate license failed. Status: %d", status);
                return 0;
            }
        }
        else
    ...
    if (argc == 2)
    {
        ...
        {
            printf("Command for generating offline req : ./sample --generate-offline-activation-request --request-file-path ./req.txt --license-key XXXX \n");
            printf("Command for Activating offline : ./sample --activate-offline --response-file-path ./res.dat");
            return 0;
        }
    }
